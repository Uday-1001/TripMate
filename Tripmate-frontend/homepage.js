...
let stellaBookingState = null;
let bookingConversationState = null;
let bookingHistory = [];
let visitedPlaces = [];
let conversationHistory = [];
let userLocation = null;

// Offers and coupons (will load from API)
const offers = [
...
// Destinations will be loaded from API
let destinations = [];

// Function to load destinations from backend
async function loadDestinations() {
  try {
    const response = await API.Destinations.getAll();
    destinations = response.destinations || response;
    console.log("✅ Destinations loaded from backend:", destinations);
    // Re-render destinations grid if it exists
...
function completeBooking() {
  ...
  // Track visited place with price
  addVisitedPlace(booking.destination, booking.image, booking.price);
...
function addVisitedPlace(destination, image, price = 0) {
  const visited = visitedPlaces.find((v) => v.name === destination);
  if (!visited) {
    visitedPlaces.push({
      name: destination,
      image,
...
function loadUserProfile() {
  ...
  const userEmail = localStorage.getItem("tripmate_token")
  ...
  document.getElementById("profileUserPhone").textContent =
    localStorage.getItem("user_phone") || "+1-234-567-8900";
  document.getElementById("profileUserLocation").textContent =
    localStorage.getItem("user_location") || "Not specified";
}
...
function toggleProfileDropdown() {
  ...
  if (menu.classList.contains("active")) {
    // Add pulse animation to icon when opening
...
function editProfile() {
  ...
  const userEmail = document.getElementById("profileUserEmail").textContent;
  const userPhone = document.getElementById("profileUserPhone").textContent;
  const userLocation = document.getElementById(
    "profileUserLocation",
  ).textContent;
  ...
  document.getElementById("editLocation").value = userLocation;
...
function loadSettings() {
  ...
  const smsNotif = localStorage.getItem("smsNotifications") !== "false";
  const promoEmails = localStorage.getItem("promotionalEmails") === "true";
  ...
  document.getElementById("promotionalEmails").checked = promoEmails;
  document.getElementById("languagePreference").value = language;
  document.getElementById("currencyPreference").value = currency;
}
...
function saveSettings() {
  ...
  localStorage.setItem(
    "promotionalEmails",
    document.getElementById("promotionalEmails").checked,
  );
  localStorage.setItem(
    "language",
  ...
  localStorage.setItem(
    "currency",
    document.getElementById("currencyPreference").value,
  );

  alert("⚙️ Settings saved successfully!");
  closeProfileModal("settingsModal");
}
...
function enable2FA() {
  alert(
    "🔒 Two-Factor Authentication will be enabled shortly. Check your email for verification code.",
  );
...
function addPaymentMethod() {
  ...
  if (cardNumber) {
    ...
    paymentMethods.push(`💳 ${cardNumber}`);
    ...
    newMethod.innerHTML = `
      <span>💳 Card ending in ${cardNumber}</span>
      <button onclick="removePaymentMethod()">Remove</button>
    `;
    paymentList.appendChild(newMethod);
  }
}
...
function loadWishlist() {
  const wishlist = visitedPlaces || [];
  const wishlistGrid = document.getElementById("wishlistGrid");

  if (wishlist.length === 0) {
    wishlistGrid.innerHTML =
      '<p class="empty-message">Your wishlist is empty. Start adding destinations!</p>';
  } else {
    wishlistGrid.innerHTML = wishlist
      .map((place, index) => {
        // Handle both old format (destination) and new format (name)
        const placeName = place.name || place.destination || "Unknown Place";
        const placePrice = place.price || 0;
        const placeImage = place.image || "https://via.placeholder.com/150";
...
function loadMyBookings() {
  const bookings = bookingHistory || cart || [];
  const bookingsList = document.getElementById("bookingsList");

  if (bookings.length === 0) {
    bookingsList.innerHTML =
      '<p class="empty-message">No bookings yet. Start booking your adventure!</p>';
  } else {
    bookingsList.innerHTML = bookings
      .map(
        (booking, index) => `
      <div class="booking-card">
        <h4>${booking.destination || booking.name || "Destination"}</h4>
        <p>📅 Guests: ${booking.guests || 1}</p>
        <p>💰 Total: $${booking.totalPrice || booking.price || 0}</p>
        <p>⏰ Status: ${booking.status || "Pending"}</p>
        <button onclick="viewBookingDetails(${index})" class="btn-view">View Details</button>
      </div>
    `,
      )
      .join("");
...
function viewBookingDetails(index) {
  alert(
    "📋 Booking Details:\nDestination: " +
      (bookingHistory[index]?.destination || "N/A") +
      "\nStatus: Confirmed",
  );
}
